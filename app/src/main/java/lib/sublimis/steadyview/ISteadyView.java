...
{
	AccessibilityAction STEADY_MARK = new AccessibilityAction(1000573491, "(SteadyView)");
	AccessibilityAction STEADY_ACTION = new AccessibilityAction(1000573492, "(SteadyViewAction)");
	...
	default boolean setSteadyViewEnabled(final boolean enabled)
	{
		...
		if (retVal)
		{
			final SteadyAccDelegate steadyAccDelegate = getSteadyAccDelegate();

			if (enabled)
			{
				if (steadyAccDelegate != null)
				{
					steadyAccDelegate.enable();
				}
			}
			else
			{
	...
	default SteadyAccDelegate getSteadyAccDelegate()
	{
		return null;
	}

	default void setSteadyAccDelegate(SteadyAccDelegate steadyAccDelegate)
	{
	}
}
