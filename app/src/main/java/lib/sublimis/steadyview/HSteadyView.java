...
public class HSteadyView
{
	...
	public static void initSteadyView(@NonNull final ISteadyView steadyView)
	{
		...
		{
			final SteadyAccDelegate steadyAccDelegate = new SteadyAccDelegate()
			{
				@Override
				public void onInitializeAccessibilityNodeInfo(@NonNull final View host, @NonNull final AccessibilityNodeInfo info)
				{
					...
					info.addAction(ISteadyView.STEADY_MARK);
					info.setText(ISteadyView.STEADY_MARK.getLabel());

					if (steadyView.isSteadyViewEnabled())
					{
						info.addAction(ISteadyView.STEADY_ACTION);
					}

					this.mNodeRef.set(info);
				}
			};

			steadyView.setSteadyAccDelegate(steadyAccDelegate);
			((View) steadyView).setAccessibilityDelegate(steadyAccDelegate);
	...
	public static class SteadyAccDelegate extends AccessibilityDelegate
	{
		protected final AtomicReference<AccessibilityNodeInfo> mNodeRef = new AtomicReference<>(null);

		public void enable()
		{
			final AccessibilityNodeInfo node = mNodeRef.get();

			if (node != null)
			{
				node.addAction(ISteadyView.STEADY_ACTION);
			}
		}
		...
		public void disable()
		{
			...
			if (node != null)
			{
				node.removeAction(ISteadyView.STEADY_ACTION);
			}
		}
	}
}
